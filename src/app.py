...
def load_teacher_credentials() -> dict[str, str]:
    ...
    if not isinstance(credentials, dict):
        raise RuntimeError("teachers.json must contain an object of username/password pairs")

    normalized_credentials: dict[str, str] = {}
    for username, password in credentials.items():
        if isinstance(username, str) and isinstance(password, str):
            normalized_credentials[username] = password
...
def require_teacher(x_teacher_token: str | None) -> str:
    if x_teacher_token is None or x_teacher_token not in active_teacher_tokens:
        raise HTTPException(
            status_code=401,
...
def teacher_login(payload: TeacherLoginRequest):
    ...
    active_teacher_tokens[token] = payload.username
    return {
...
def teacher_logout(x_teacher_token: str | None = Header(default=None, alias="X-Teacher-Token")):
    ...
    active_teacher_tokens.pop(x_teacher_token, None)
